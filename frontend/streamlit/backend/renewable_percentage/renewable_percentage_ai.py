...
def when_to_consume_energy_RP(prediction_class_renewable, mode_labelling_RP):
    if (
        prediction_class_renewable > mode_labelling_RP
        and prediction_class_renewable > 3
    ):
        ...
        prediction_class_renewable == mode_labelling_RP
        and mode_labelling_RP > 3
        or prediction_class_renewable > 3
    ):
...
def render_ai_predictions_RP():
    ...
    labelling_rp = labelling_scaler_rp.transform(
        renewable_percentage_values.reshape(-1, 1)
    )
    ...
    with col_current:
        value_displayed_now, relative_value_now = colored_metric(
            "Last 24 hours", mode_labelling_RP, bg_color_carbon
        )
    with col_arrow:
    ...
    with col_prediction:
        value_displayed_next, relative_value_next = colored_metric(
            "Next 24 hours", prediction_class_renewable, bg_color_renewable
        )

    timming_message = when_to_consume_energy_RP(
        prediction_class_renewable, mode_labelling_RP
    )
