...
def create_carbon_intensity_report_download_button(data, charts=None, title="Carbon Intensity Portugal Overview"):
    """
    Creates a download button for a comprehensive carbon intensity report in PDF format.
    
    Args:
        data (pd.DataFrame): The carbon intensity data to include in the report
...
def generate_carbon_intensity_pdf_report(data, charts=None, title="Carbon Intensity Portugal Overview"):
    ...
    Args:
        data (pd.DataFrame): The carbon intensity data
    ...
    if not data.empty:
        ...
        stats_text = (
            f"Average Carbon Intensity: {stats.loc['mean'].iloc[0]} gCO2eq/kWh\n"
            f"Minimum Carbon Intensity: {stats.loc['min'].iloc[0]} gCO2eq/kWh\n"
            f"Maximum Carbon Intensity: {stats.loc['max'].iloc[0]} gCO2eq/kWh\n"
